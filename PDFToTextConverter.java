...
public class PDFToTextConverter {
    public static void main(String[] args) throws ParseException, IOException {
        //thetoume arxikes parametrous gia ta path sto current working directory
        String source_path = System.getProperty("user.dir");
        String dest_path = System.getProperty("user.dir");
        
        //an uparxei 1 parametros tote to source_path pairnei tin timi tou
        if(args.length == 1) source_path = args[0];
        
        
        //an uparxei kai 2i parametros tote to dest_path pairnei tin timi tou
        if (args.length == 2) {
            source_path = args[0];
            dest_path = args[1];
        }
        ...
        File dest_folder = new File(dest_path);
        if (!dest_folder.isDirectory()) new File(dest_path).mkdirs();
